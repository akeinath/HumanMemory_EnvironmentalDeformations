function data = getDataVive(folder)
    snames = dir(folder);
    ...
    for si = snames
        ...
        for fi = 1:length(files)
            ...
            segmentedData(fi,:) = segment(1:48);
        end
        fclose all;
        replaceTrials = segmentedData(:,18:4:end);
        replaceTrials = [replaceTrials(1,:) replaceTrials(2,:); ...
            replaceTrials(3,:) replaceTrials(4,:)];
        ...
        for gi = 1:2
            for ti = 1:16
                ...
                if ismember(replaceTrials{gi,1}{1},{'Compress,'})
                    centerOfRoom = [0.8 0.2];
                elseif ismember(replaceTrials{gi,1}{1},{'Stretch,'})
                    centerOfRoom = [0.4 0.2];
                else
                    centerOfRoom = [0.6 0.2];
                end
                
                path(:,1:2) = bsxfun(@minus,path(:,1:2),centerOfRoom);
                if any(ismember(ct,{'Rotated'}))
                    path(:,1:2) = path(:,1:2) * [cos(pi) -sin(pi); sin(pi) cos(pi)];
                end
%                 plot(path(:,1),path(:,2))
%                 hold on
